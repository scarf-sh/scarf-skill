...
- Aggregation helper: explicit rollup (including `total`), artifact selectors, filters, grouping, and JSON/NDJSON; see [Aggregation queries](references/spec.md#aggregation-queries)
- Page/company analytics: raw-referrer or configured-variable scope; exact URI-path matching for referrers; server-side artifact grouping, without client row deduplication
...
- `ruby scripts/test_aggregation_export.rb`: validate aggregation selectors, grouping, JSON/NDJSON parsing, exact URI paths, rollups, company-segment constraints, and token safety.
